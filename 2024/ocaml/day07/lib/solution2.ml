...
let concatenate_ints a b = int_of_string @@ string_of_int a ^ string_of_int b
...
let is_valid_equation state =
  let rec aux queue =
    ...
    | { value; partial_result; elements } :: xs ->
      ...
      let concat_state =
        { value
        ; partial_result = concatenate_ints partial_result (List.hd elements)
        ; elements = List.tl elements
        }
      in
      aux (plus_state :: mult_state :: concat_state :: xs)
  in
  aux [ state ]
;;
